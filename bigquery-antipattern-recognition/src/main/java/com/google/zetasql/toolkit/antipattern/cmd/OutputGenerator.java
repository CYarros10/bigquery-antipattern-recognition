...
public class OutputGenerator {
    ...
    if (cmdParser.hasOutputFileOptionName()) {
      ...
      for (OutputResult result : outputData) {
        for (RecommendOutput RecOut : result.getListRecommend()) {
          System.out.println("--------------------------------------");
          System.out.println(String.join(";", new String[] {result.getJobId(),result.getQuery(),String.valueOf(result.getTotalSlotMs()),RecOut.getName(),RecOut.getDescription()}));
        }
      }
  }
  }
  ...
  private static void writeOutputToCSV(List<OutputResult> outputData, BQAntiPatternCMDParser cmdParser)
    ...
    for (OutputResult result : outputData) {
        try {
              for (RecommendOutput RecOut : result.getListRecommend()) {
                  csvWriter.write(String.join(";", new String[] {result.getJobId(),result.getQuery(),String.valueOf(result.getTotalSlotMs()),RecOut.getName(),RecOut.getDescription()}));
              }
        }
        catch (Exception e) {
          e.printStackTrace();
  ...
  private static void writeOutputToBQTable(
    ...
    for (OutputResult result : outputData) {
      for (RecommendOutput RecOut : result.getListRecommend()) {
        ...
        rowContent.put("total_slot_ms", result.getTotalSlotMs());
